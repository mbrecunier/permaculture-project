...
public class User {
  ...
  public void removePlant(int plantId) {
    try(Connection con = DB.sql2o.open()) {
      String sql = "DELETE FROM users_plants WHERE user_id = :user_id AND plant_id = :plant_id;";
      con.createQuery(sql)
        .addParameter("user_id", this.id)
        .addParameter("plant_id", plantId)
        .executeUpdate();
  ...
  public void removeTool(int toolId) {
    try(Connection con = DB.sql2o.open()) {
      String sql = "DELETE FROM users_tools WHERE id IN (SELECT id FROM users_tools WHERE user_id = :user_id AND tool_id = :tool_id LIMIT 1)";
      con.createQuery(sql)
        .addParameter("user_id", this.id)
        .addParameter("tool_id", toolId)
        .executeUpdate();
